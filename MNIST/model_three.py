...
class Dehaze(nn.Module):
    def __init__(self, input_nc,  output_nc, use_dropout=False, padding_type='reflect'):
        ...
        self.down1 = nn.Sequential(nn.Conv2d(input_nc, 64, kernel_size=5, stride=1, padding=2),
                                   nn.BatchNorm2d(64),
                                   nn.ReLU(True))
        self.down2 = nn.Sequential(nn.Conv2d(64, 128, kernel_size=4, stride=2, padding=1), #original：4， 2， 3
                                   nn.BatchNorm2d(128),
                                   nn.ReLU(True))
        ...
        self.up2 = nn.Sequential(nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True),
                                 ...
                                 nn.BatchNorm2d(64),
                                 nn.ReLU(True))
        self.up3 = nn.Sequential(nn.Conv2d(64, output_nc, kernel_size=3, padding=1),
                                 nn.BatchNorm2d(2),
                                 nn.ReLU(True))
        self.outc = nn.Conv2d(2, 1, kernel_size=1)
