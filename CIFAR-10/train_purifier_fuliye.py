...
def train(net, loader_train, loader_test, optim, criterion, scheduler):
    ...
    for epoch in range(1, epochs + 1):
        for batch_idx, (images, labels, adv_data) in enumerate(loader_train):
            ...
            loss = loss_rec*10 + loss_vgg7*5 + loss_pinglv + pertual_loss
            loss.backward()
            ...
            loss3.append(loss_pinglv.item())
            loss4.append(pertual_loss.item())

        scheduler.step()

        print("epoch {}'s: loss:{:.5f}, rec_losses:{:.5f}, vgg_losses:{:.5f}, loss3:{:.5f}, loss4:{:.5f}".format(epoch, np.sum(losses) / len(losses), \
...
if __name__ == "__main__":
    ...
    model_dir = './Cifar10_final_model2'
    if not os.path.exists(model_dir):
        os.mkdir(model_dir)
    model_name = 'test2'
    epochs = 100
    lr = 0.002 #(0.0001, 0.001)
