...
def train(net, loader_train, loader_test, optim, criterion, scheduler):
    ...
    for epoch in range(1, epochs + 1):
        for batch_idx, (images, labels, adv_data) in enumerate(loader_train):
            ...
            loss = loss_rec + loss_vgg7 + pertual_loss + loss_pinglv
            loss.backward()
...
def imgshow1(img):
    ...
    plt.imshow(np.transpose(npimg, (1, 2, 0)))
    plt.show()
    plt.savefig('./caltech101_orig.png')
    print('image already saved!')
    return
...
def test(epoch, net, loader_test):
    ...
    for i, (images, labels) in enumerate(loader_test):
        ...
        y = labels.cuda()
        with torch.no_grad():
            out, final_pred = net(inputs)

            # out = torchvision.utils.make_grid(inputs)
            # out1 = torchvision.utils.make_grid(final_pred)
            # # imgshow1(out)
            # imgshow(out1)
